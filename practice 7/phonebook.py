...
class PhoneBook:
            ...
            with open(csv_file_path, "r", encoding="utf-8") as file:
                ...
                for row in csv_reader:
                    if len(row) >= 3:
                        ...
                        db.execute_query(
                            insert_query, (first_name, last_name, phone_number, email)
                        )
                        result = db.fetch_one()
                ...
                db.commit()
                print(
                    f"✓ Successfully processed: {inserted} new, {updated} updated contacts"
                )
                return True
    ...
    def update_contact(self):
        ...
        if new_phone:
            ...
            check_query = (
                "SELECT id FROM phonebook WHERE phone_number = %s AND id != %s"
            )
    ...
    def delete_contact(self):
        ...
        confirm = (
            input(
                f"\nDelete contact {contact['first_name']} {contact['last_name'] or ''}? (y/n): "
            )
            .strip()
            .lower()
        )
    ...
    def display_contacts(self, contacts):
        ...
        print("\n" + "=" * 100)
        print(
            f"{'ID':<5} {'First Name':<20} {'Last Name':<20} {'Phone Number':<15} {'Email':<30}"
        )
        print("=" * 100)
